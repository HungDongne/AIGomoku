...
class GameClient:
    ...
    def convert_char_to_num(self, role):
        size = int(self.size)
        maxtrix = []
        for i in range(size):
            row = []
            for j in range(size):
                if self.board[i][j] == role:
                    row.append(1)
                elif self.board[i][j] == " ":
                    row.append(0)
                else:
        ...
        while not stop_thread:
            ...
            # Nhận thông tin trò chơi
                ...
                if data.get("turn") in self.team_id:
                    ...
                    move = (0, 0)
                    # Nếu được đi trước (là đội x) thì gửi nước đi đầu tiên
                    if (self.START_FIRST):
                        move = self.FIRST_MOVE()
                    else:
                        tempBoard = self.convert_char_to_num(self.team_roles)
                        AIBoard = Board(self.size)
                        AIBoard.board_matrix = tempBoard
...
if __name__ == "__main__":
    ...
    if (team_roles == 'x'): gameClient.START_FIRST = True
    else: gameClient.START_FIRST = False
    gameClient.listen()
    # game = GameClient(host, "25", "team-1", "team-2", "x")
    # game.size = 15
    # print(game.FIRST_MOVE())
    # print(game.START_FIRST)
    # if (not game.START_FIRST):
    #     print("Turn 2:")
